...
func (kv *ShardKV) Get(args *GetArgs, reply *GetReply) {
	...
	// 判断该命令是否已经被执行过了
	if result, ok := kv.clientSeq[args.ClientId]; ok && result.RequestId >= args.RequestId {
		...
		go func(shard int, config shardctrler.Config) {
			...
			for _, server := range config.Groups[gId] {
				...
				if ok && reply.Success {
					...
					kv.rf.Start(GCSuccessCommand{
						ConfigNum: config.Num,
						Shard:     shard,
					})
					break
				}
				DPrintf("error push gc signal")
			}
		}(shard, c)
	}
	kv.mu.Unlock()
		...
		go func(shard int, config shardctrler.Config) {
			...
			for _, server := range config.Groups[gId] {
				...
				if ok && reply.Err == OK {
					...
					kv.rf.Start(ShardReplicaCommand{
...
func (kv *ShardKV) updateConfig(config shardctrler.Config) {
	...
	for shard, gId := range config.Shards {
		...
		if _, ok := needSendShards[shard]; ok || oldConfig.Num == 0 {
			...
			// 从需要发送给别的组的分片集合中删掉当前仍负责的分片,因为上一个配置也是自己负责的,因为不需要发送给别的节点
...
func (kv *ShardKV) needSnapshot() bool {
	...
	return float32(kv.rf.GetRaftStateSize()/kv.maxraftstate) > 0.9
}
