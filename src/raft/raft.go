...
// rf.Start(command interface{}) (getLogByIndex, term, isleader)
...
type Raft struct {
	...
	me        int                 // this peer's getLogByIndex into peers[]
...
// all info up to and including getLogByIndex. this means the
// service no longer needs the logEntries through (and including)
// that getLogByIndex. Raft should now trim its logEntries as much as possible.
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	// Your code here (2D).
	rf.mu.Lock()
	defer rf.mu.Unlock()
	if index > rf.getLastLog().Index || index < rf.logEntries[0].Index || index > rf.commitIndex {
		return
	}
	//1.获取需要压缩末尾日志的数组内索引
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if rf.getLastLog().Term == -1 || args.LastLogTerm > rf.getLastLog().Term || (args.LastLogTerm == rf.getLastLog().Term && args.LastLogIndex >= rf.getLastLog().Index) {
		//重置选举时间
		rf.resetElectTimer()
		//投票给候选人
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if rf.logEntries[0].Index <= args.PrevLogIndex && rf.getLogByIndex(args.PrevLogIndex).Term != args.PrevLogTerm {
		reply.Success = false
		//更新XTerm为冲突的Term
		reply.XTerm = rf.getLogByIndex(args.PrevLogIndex).Term
	...
	for i, logEntry := range args.Entries {
		...
		if index > rf.getLastLog().Index {
			...
			if rf.getLogByIndex(index).Term != logEntry.Term {
	...
	//for getLogByIndex, entry := range args.Entries {
	//	if entry.Index-firstIndex >= len(rf.logEntries) || rf.logEntries[entry.Index-firstIndex].Term != entry.Term {
	//		rf.logEntries = append(rf.logEntries[:entry.Index-firstIndex], args.Entries[getLogByIndex:]...)
	//		break
	//	}
	//}
...
func (rf *Raft) updateCommitIndexForFollower(leaderCommit int) {
	...
	if leaderCommit > rf.commitIndex {
		if leaderCommit > rf.getLastLog().Index {
...
// server is the getLogByIndex of the target server in rf.peers[].
...
//
// the first return value is the getLogByIndex that the command will appear at
	...
	for !rf.killed() {
		...
		for i := rf.getLastLog().Index; i > rf.commitIndex; i-- {
			...
			for j := range rf.peers {
				...
				if rf.matchIndex[j] >= i && rf.getLogByIndex(i).Term == rf.currentTerm {
					num++
				}
			}
			//若过半数则更新commitIndex
...
func (rf *Raft) HandleAppendEntries(server int) {
	...
	args := AppendEntriesArgs{
		...
		PrevLogTerm:  rf.getLogByIndex(rf.nextIndex[server] - 1).Term,
